...
public class UnSynchronizedFilter : IParameterSettable
{
    ...
    public ParameterSet ToParameterSet()
    {
        ...
        var sql = $"not exists (select * from {KeyMapTableName} {KeyMapAliasName} where {alias}.{keys.Select(x => $"{alias}.{x} = {KeyMapAliasName}.{x}")})";
